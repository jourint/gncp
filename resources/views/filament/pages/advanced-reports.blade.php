<x-filament-panels::page>
    <div class="space-y-4">
            ...
            <div class="text-sm">
                @forelse($this->getCuttingReportProperty() as $item)
                    @if($item['type'] === 'tech_card_header')
                        <div class="mb-1"><span class="font-bold">{{ $item['model_name'] }}</span> — {{ $item['tech_card_name'] }} (итого: <span class="font-bold">{{ $item['total_quantity'] }}</span>)</div>
                        <div class="ml-4 mb-2 text-gray-600">
                            @foreach($item['sizes'] as $size)
                                {{ $size->size_id }}: {{ $size->total_quantity }};
                            @endforeach
                        </div>
                    @elseif($item['type'] === 'model_total')
                        <div class="mb-2 mt-1 font-bold border-t pt-1">Итого {{ $item['model_name'] }}: {{ $item['total_quantity'] }}</div>
                    @elseif($item['type'] === 'overall_total')
                        <div class="mt-3 font-bold text-lg border-t pt-1">Общий итог: {{ $item['total_quantity'] }}</div>
                    @endif
                @empty
                    <p>Нет данных</p>
                @endforelse
        ...
        <div id="sewing-report" class="p-4 bg-white rounded border">
            ...
            <div class="text-sm">
                @forelse($this->getSewingReportProperty() as $item)
                    @if($item['type'] === 'tech_card_header')
                        <div class="mb-1"><span class="font-bold">{{ $item['employee_name'] }}</span> — {{ $item['tech_card_name'] }} (итого: <span class="font-bold">{{ $item['total_quantity'] }}</span>)</div>
                        <div class="ml-4 mb-2 text-gray-600">
                            @foreach($item['sizes'] as $size)
                                {{ $size->size_id }}: {{ $size->total_quantity }};
                            @endforeach
                    ...
                    @elseif($item['type'] === 'employee_total')
                        <div class="mb-2 mt-1 font-bold border-t pt-1">Итого {{ $item['employee_name'] }}: {{ $item['total_quantity'] }}</div>
                    @elseif($item['type'] === 'overall_total')
                        <div class="mt-3 font-bold text-lg border-t pt-1">Общий итог: {{ $item['total_quantity'] }}</div>
                    @endif
                @empty
        ...
        <div id="miscellaneous-report" class="p-4 bg-white rounded border">
            ...
            <div class="text-sm">
                <div class="mb-3">
                    <h3 class="font-bold">Стельки:</h3>
                    @php
                        $groupedStelki = $this->getMiscellaneousReportProperty()['stelki']->groupBy(['name', 'is_black']);
                    @endphp
                    @forelse($groupedStelki as $name => $byColor)
                        @foreach($byColor as $isBlack => $items)
                            <div>
                                <span class="font-bold">{{ $name }} ({{ $isBlack ? 'Черная' : 'Бежевая' }})</span> —
                ...
                <div class="mb-3">
                    <h3 class="font-bold">Подноски:</h3>
                    @php
                    ...
                    @forelse($puffs as $puff)
                        <div>{{ $puff['name'] }} — {{ $puff['total'] }}</div>
                    @empty
                        <p>Нет данных</p>
                    @endforelse

                    <h3 class="font-bold mt-2">Задники:</h3>
                    @php
                    ...
                    @forelse($counters as $counter)
                        <div>{{ $counter['name'] }} — {{ $counter['total'] }}</div>
                    @empty
                        <p>Нет данных</p>
                    @endforelse
                </div>
                ...
                <div>
                    <h3 class="font-bold">Доп. работы:</h3>
                    @forelse($this->getMiscellaneousReportProperty()['workflows'] as $wf)
                        <div>{{ $wf['name'] }} — {{ $wf['total_quantity'] }} пар</div>
                    @empty
                        <p>Нет данных</p>
                    @endforelse
                </div>
            ...
            <div class="text-sm">
                @forelse($this->getExpeditionReportProperty() as $item)
                    @if($item['type'] === 'model_header')
                        <div class="mb-1">
                            <span class="font-bold">{{ $item['customer_name'] }}</span> — {{ $item['tech_card_name'] }} (итого: <span class="font-bold">{{ $item['total_quantity'] }}</span>)
                        </div>
                        <div class="ml-4 mb-2 text-gray-600">
                            @foreach($item['sizes'] as $size)
                                {{ $size->size_id }}: {{ $size->total_quantity }};
                    ...
                    @elseif($item['type'] === 'customer_total')
                        <div class="mb-2 mt-1 font-bold border-t pt-1">Итого {{ $item['customer_name'] }}: {{ $item['total_quantity'] }}</div>
                    @elseif($item['type'] === 'overall_total')
                        <div class="mt-3 font-bold text-lg border-t pt-1">Общий итог: {{ $item['total_quantity'] }}</div>
                    @endif
                @empty
        ...
        <div id="stock_requirements-report" class="p-4 bg-white rounded border">
            ...
            <div class="text-sm">
                <div class="mb-4">
                    <h3 class="font-bold text-gray-700">Для кроя:</h3>
                ...
                <div>
                    <h3 class="font-bold text-gray-700">Для стелек:</h3>
